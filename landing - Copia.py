...
def landing_page():
    ...
    *,*::before,*::after{{margin:0;padding:0;box-sizing:border-box}}
    ...
    /* ═══ HERO ═══ */
    .lp-hero{{width:100%;padding:150px 0 90px;background:linear-gradient(155deg,#eff6ff 0%,#f0f9ff 40%,#fff 100%)}}
        ...
        with ui.element('div').classes('cl'):
            with ui.element('div').classes('lp-hero-grid'):
                with ui.element('div'):
                    ...
                    ui.html('<h1 class="lp-hero-h1">Domine suas finanças com <span>inteligência</span></h1>')
                    ui.label('Controle total dos seus cartões de crédito, gastos organizados por categoria e alertas personalizados para você nunca perder o controle.').classes('lp-hero-p')
                    with ui.element('div').classes('lp-hero-btns'):
                        ui.button('🚀 Começar Agora — Grátis', on_click=lambda: ui.navigate.to('/criar-conta')).props('no-caps').style('background:#2563eb!important;color:#fff!important;font-weight:600;font-size:15px;padding:14px 28px!important;border-radius:10px;box-shadow:0 4px 14px rgba(37,99,235,.32)')
                    ...
                    with ui.element('div').classes('lp-hero-trust'):
                        for txt in ['Grátis para sempre', 'Sem cartão de crédito', '2 minutos para começar']:
                            with ui.element('span'):
                                ui.label('✓').style('color:#2563eb!important;font-size:15px')
                                ui.label(txt)
                with ui.element('div').classes('lp-hero-img-wrap'):
                    ui.html(f'<img src="{PRINT_DASHBOARD}" alt="Dashboard Cartometro" style="width:100%;display:block">')
