...
def build_context(dados, gastos, fixos, cartoes=None, gastos_mes_passado=None):
    ...
    if cartoes:
        for cartao in cartoes:
            ...
            gastos_cartao = sum(g.get("valor", 0) for g in gastos if g.get("cartao") == nome)
            ctx.cartoes_uso[nome] = gastos_cartao
        ctx.cartoes_sem_uso = [nome for nome, valor in ctx.cartoes_uso.items() if valor == 0]
...
REGRAS_PREMIUM = [
    'r_limite_estourado', 'r_percentual_95', 'r_parcelado_excedido',
