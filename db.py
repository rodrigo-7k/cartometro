...
def inicializar():
    ...
    if not os.path.exists(USUARIOS_FILE):
        ...
        # Criar dados iniciais do admin demo
        dados_demo = criar_dados_iniciais("Admin Demo")
        dados_demo["perfil"]["avatar_emoji"] = "👑"
        salvar_json(f'{DADOS_DIR}/admin.json', dados_demo)
...
def criar_usuario(nome, email, senha, plano="gratuito"):
    ...
    Args:
        nome: Nome completo
        email: Email (será usado como login)
        senha: Senha (mínimo 4 caracteres)
        plano: "gratuito" ou "premium"
    
    Returns:
        tuple: (sucesso, mensagem, usuario_dict)
    """
    # Validações
    ...
    # Configurar expiração para premium
...
def atualizar_plano_usuario(email, novo_plano):
    ...
    Args:
        ...
        novo_plano: "gratuito" ou "premium"
...
def atualizar_perfil_usuario(email, nome=None, avatar_emoji=None, senha=None):
    ...
    for u in usuarios:
        if u['email'].lower() == email.lower():
            ...
            # Atualizar também no arquivo de dados
...
# ============================================================
# DADOS DO USUÁRIO (GASTOS, CONFIG, ETC)
# ============================================================
def carregar_dados_usuario(email):
    """Carrega todos os dados (gastos, config, cartoes) de um usuário"""
    if not email:
...
def set_usuario_logado(email):
    """Define o usuário logado atual (para funções de compatibilidade)"""
