        ...
        with lista:
            ...
            if sem_data:
                ...
                for idx, g in sem_data:
                    categoria = g.get("categoria", "Sem categoria")
                    cor_cat, icone_cat = get_cor_icone(categoria)
                    with ui.card().classes('lancamento-card-fixo').style(f'border-left-color: {cor_cat} !important; margin-left: 5px !important; margin-right: 5px !important; border-radius: 12px !important;').on('click', lambda g=g, i=idx: editar_fixo(g, i)):
                        with ui.row().classes('items-center justify-between w-full'):
                            with ui.row().classes('items-center gap-3 flex-1'):
                                ui.icon(icone_cat).classes('text-lg').style(f'color: {cor_cat} !important;')
                                with ui.column().classes('gap-0 flex-1'):
